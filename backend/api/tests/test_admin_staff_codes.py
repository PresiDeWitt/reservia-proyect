...
class AdminStaffCodesTests(TestCase):
    def setUp(self):
        ...
        self.code = StaffCode.objects.create(code=make_password("OWNER-123"), role="owner")

    def test_list_codes(self):
        res = self.client.get("/api/admin/staff-codes/")
        self.assertEqual(res.status_code, 200)
        self.assertEqual(len(res.data["codes"]), 1)

    def test_create_code(self):
        rest = make_restaurant()
        initial_count = StaffCode.objects.count()
        res = self.client.post("/api/admin/staff-codes/", {
            "code": "NEW-456", "role": "owner", "restaurant_id": rest.id,
        }, format="json")
        self.assertEqual(res.status_code, 201)
        self.assertEqual(StaffCode.objects.count(), initial_count + 1)
