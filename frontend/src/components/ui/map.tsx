...
function DefaultLoader() {
  return (
    <div className="bg-background/50 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-[2px]">
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Auto-fly to center/zoom changes when not in controlled mode
  useEffect(() => {
    if (!mapInstance || isControlled || !mapInstance.isStyleLoaded()) return;
    if (centerProp === undefined && zoomProp === undefined) return;

    const current = getViewport(mapInstance);
    const targetCenter: [number, number] = centerProp !== undefined
      ? [MapLibreGL.LngLat.convert(centerProp).lng, MapLibreGL.LngLat.convert(centerProp).lat]
      : current.center;
    const targetZoom = zoomProp !== undefined ? zoomProp : current.zoom;

    if (
      targetCenter[0] === current.center[0] &&
      targetCenter[1] === current.center[1] &&
  ...
  }, [mapInstance, isControlled, centerProp, zoomProp]);
